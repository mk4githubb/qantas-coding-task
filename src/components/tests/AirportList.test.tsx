...
describe('AirportList', () => {
  it('shows loader when loading', () => {
    jest.spyOn(routerDom, 'useOutletContext').mockReturnValue({ data: [], isLoading: true, isError: false });
    render(<AirportList />);
    expect(screen.getByText(/loading airports/i)).toBeInTheDocument();
  });

  it('shows error when error', () => {
    jest.spyOn(routerDom, 'useOutletContext').mockReturnValue({ data: [], isLoading: false, isError: true });
    render(<AirportList />);
    expect(screen.getByText(/failed to load airports/i)).toBeInTheDocument();
  });

  it('shows empty list when no airports', () => {
    jest.spyOn(routerDom, 'useOutletContext').mockReturnValue({ data: [], isLoading: false, isError: false });
    render(<AirportList />);
  ...
  it('shows paginated airports and handles click', () => {
    ...
    jest.spyOn(routerDom, 'useOutletContext').mockReturnValue({ data: airports, isLoading: false, isError: false });
    render(<AirportList />);
